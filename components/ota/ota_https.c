...
bk_err_t https_ota_event_cb(bk_http_client_event_t *evt)
{
	...
	BK_LOGW(TAG, "HTTPS_EVENT_ON_FINISH\r\n");
	break;
...
int bk_https_ota_download(const char *url)
{
	...
	BK_LOGW("current_partition :0x%x",current_partition);
	...
	if (client == NULL) {
		BK_LOGW(TAG, "client is NULL\r\n");
		err = BK_FAIL;
		return err;
	}
	err = bk_http_client_perform(client);
	if(err == BK_OK){
		BK_LOGW(TAG, "bk_http_client_perform ok\r\n");
        ...
        BK_LOGW(TAG, "from cus temp_exec_flag:0x%x \r\n",temp_exec_flag);
        ...
        BK_LOGW(TAG, "temp_exec_flag:0x%x \r\n",exec_temp_part);
	 ...
	 bk_reboot();
		...
		BK_LOGW(TAG, "bk_http_client_perform fail, err:%x\r\n", err);
	}
