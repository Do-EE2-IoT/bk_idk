...
bk_err_t websocket_send_binary(const uint8_t *data, int len)
{
	...
	{
		transport client = (transport)websocket_run;
		if (websocket_client_send_with_opcode(client, WS_TRANSPORT_OPCODES_BINARY, data, len, WEBSOCKET_NETWORK_TIMEOUT_MS) < 0)
		{
			BK_LOGE(TAG, "websocket_send_binary fail\r\n");
			return BK_FAIL;
		}
		return BK_OK;
	}
